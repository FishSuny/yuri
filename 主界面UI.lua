...
m_onChassisWinDownjineng = false
...
function onSkillUp(e)
	...
	if ui:findComponent("youxia,玩家怪物") and g==ui:findComponent("youxia,Skill_box_1") then
		...
		-- 获取技能释放位置坐标
        --local skillX, skillY = e:getLocalPos().x, e:getLocalPos().y
	    --local skillshp = F3DShape:new()
	    --skillshp:drawCircle(skillX, skillY, g_skill[g.id].range - RANGEOFFSET, 0xFF0000FF, true)  
        --skillshp:drawCircle(g_mobileMode and F3DPoint:new(30,30) or F3DPoint:new(16,16), g_mobileMode and 30 or 16)
...
function checkYouxiaSkillbgTouchable()
    print(txt("检查按下"))
	if not F3DUIManager.sTouchComp or F3DUIManager.sTouchComp == ui.skillbg then
		return true
	end
	local p = F3DUIManager.sTouchComp:getParent()
	while p and p ~= ui.skillbg do
		p = p:getParent()
	end
	return p == ui.skillbg
end
...
--给技能按钮加入滑动按钮实现拖动施法
function onChassisDownjineng(e)
    print(txt("技能按键-按下"))
...
--给技能按钮加入滑动按钮实现拖动施法
function onChassisMovejineng(e)
    print(txt("技能按键-滑动"))
...
--给技能按钮加入滑动按钮实现拖动施法
function onChassisWinDownjineng(e)
	if not checkYouxiaSkillbgTouchable() then
		return
	end
    ...
    print(txt("技能按键-按下2"))
			...
			--按下技能按钮时候根据技能范围数据自动画圆 =施法范围画圆
local shpCircle = F3DShape:new()

--local x, y = math.floor(g_role:getPositionX() / 25), math.floor(to2d(g_role:getPositionY()) / 25)
--local posx = x * 48 + 24
--local posy = -y * 48 - 24
--local 半径 = 130
...
--shpCircle:setPenSize(1)             -- 线条粗细
--shpCircle:setPenColor(50, 255, 30)  -- 线条颜色
--shpCircle:drawCircle(F3DPoint:new(x,y), 半径, false)
--shpCircle:setPosition(x,to3d(y),0)
--shpCircle:setZOrder(-math.floor(g_role:getPositionY()/25)*10000+math.floor(g_role:getPositionX()/25))

-- 将绘制的圆形添加到场景中进行显示并调整 Z 轴深度
--rolecont:addChild(shpCircle)
			local pos = getMousePoint(g_hoverPos.x, g_hoverPos.y)
			local 预警es = g_role:setEffectSystem(全局设置.getAnimPackUrl(880000), false, nil, nil, 0, -1)
			--es:setScale(1, 1, 1)    --只能3d模式用 reset()
			预警es:setScaleX(-1)
            预警es:setScaleY(-1)           --可添加放大倍数
			if 预警es then
				预警es:setPositionX(g_role:getPositionX())
				预警es:setPositionY(g_role:getPositionY())
				 
				--es:setAnimRotationZ(-30)           -- 特效在Z轴方向上逆时针旋转30度
				--es:setPositionZ(g_scene and g_scene:getTerrainHeight(pos.x, pos.y) or 0)
			end
...
function onChassisWinMovejineng(e)
...
function onChassisWinUpjineng(e)
   --m_onChassisWinDownjineng = false
   print(txt("技能按键-抬起2"))
...
sound=F3DSoundManager:instance():playSound("/res/sound/"..(游戏设置.tuiguangyl or "").."",false)
	...
	ui.zhanli = ui:findComponent("xia,zhan_shuzhi"):getBackground()--战力
		...
		--给技能按钮加入滑动按钮实现拖动施法
		ui.jinengzuozuoxia = ui:findComponent("zuozuoxia")
		
		--尝试新增功能
		--人物 目标   
		--拖动技能按钮 有偏移数据时候  进入拖动施法  若垂直无偏移 按下直接施法技能
		--拖动技能按钮 以人物为起点画一条指示箭头  画出人物当前技能按钮的施法范围 并且指示箭头方向和技能施法方向根据拖动方向自动实时变动 松开技能按钮 施放技能
		...
		if __PLATFORM__ == "WIN" or __PLATFORM__ == "MAC" then   --PC端代码
			...
			--给技能按钮加入滑动按钮实现拖动施法
			if ui.jinengzuozuoxia then
				ui.zuozuoxia:addEventListener(F3DMouseEvent.MOUSE_DOWN, func_me(onChassisWinDownjineng))
				ui.zuozuoxia:addEventListener(F3DMouseEvent.MOUSE_MOVE, func_me(onChassisWinMovejineng))
				ui.zuozuoxia:addEventListener(F3DMouseEvent.MOUSE_UP, func_me(onChassisWinUpjineng))
			end
			...
			--给技能按钮加入滑动按钮实现拖动施法
			if ui.jinengzuozuoxia then
				F3DTouchProcessor:instance():addEventListener(F3DTouchEvent.BEGIN, func_te(onChassisDownjineng))
				F3DTouchProcessor:instance():addEventListener(F3DTouchEvent.MOVE, func_te(onChassisMovejineng))
				F3DTouchProcessor:instance():addEventListener(F3DTouchEvent.END, func_te(onChassisUpjineng))
			end
		end
	...
	for i=0,7 do
		...
		if not g_mobileMode then
			...
			--加入按钮绑定操作
			--
			skillbg:addEventListener(F3DMouseEvent.MOUSE_DOWN, func_me(onChassisWinDownjineng))
			skillbg:addEventListener(F3DMouseEvent.MOUSE_MOVE, func_me(onChassisWinMovejineng))
			skillbg:addEventListener(F3DMouseEvent.MOUSE_UP, func_me(onChassisWinUpjineng))
		end
	...
	if ui.button_out then
		ui.button_out:addEventListener(F3DMouseEvent.CLICK, func_me(function (e)		
			消息框UI1.initUI()
			消息框UI1.setData(txt("是否退出到登录界面?"), function()
				消息.CG_BACK_SELECT()
				releaseScene()
				willBackSelect()
				网络连接.closeConnect()
